...
class PreAlert(AbstractElement):
	...
	def add_order(self, dispatch_number: Optional[str], number: Optional[str] = None) -> SubElement:
		"""
		Добавление заказа к preAlert
...
class CallCourier(AbstractElement):
	...
	def __init__(self, call_count: int = 1):
		...
		self.call_courier_element = ElementTree.Element(
			'CallCourier',
			CallCount=str(call_count),
		)

	def add_call(
			self, date: datetime.date, time_begin: datetime.time, time_end: datetime.time,
		...
		:param str sender_phone: телефон отправителя

		:param str sender_name: ФИО отправителя
		...
		call_element.attrib['Comment'] = str(comment)
		call_element.attrib['IgnoreTime'] = str(ignore_time)
		if lunch_begin:
	...
	@staticmethod
	def add_address(call_element: SubElement, address_street: str, address_house: str, address_flat: str) -> SubElement:
		"""
...
class DeliveryRequest(AbstractElement):
	...
	def __init__(self, number: str, order_count: int = 1):
		...
		self.delivery_request_element = ElementTree.Element(
			'DeliveryRequest',
			Number=number,
			OrderCount=str(order_count))

	def add_order(
			self, number: str, tariff_type_code: int,
		...
		:param str seller_name: Истинный продавец. Используется при печати заказов для отображения настоящего
		продавца товара, торгового названия.
		...
		order_element.attrib['SendCityCode'] = str(send_city_code)
		order_element.attrib['SendCityPostCode'] = send_city_post_code
		order_element.attrib['RecCityCode'] = str(rec_city_code)
		order_element.attrib['RecCityPostCode'] = rec_city_post_code
		order_element.attrib['RecipientName'] = recipient_name
		order_element.attrib['TariffTypeCode'] = str(tariff_type_code)
		order_element.attrib['DeliveryRecipientCost'] = str(shipping_price)
	...
	def add_address(
		...
		:param str pvz_code: Код ПВЗ. Атрибут необходим только для заказов с режимом доставки «до склада»
	...
	def add_package(
		...
		"""
		Добавление посылки. Габариты упаковки заполняются только если указаны все три значения.
		...
		:param number: Номер упаковки (можно использовать порядковый номер упаковки заказа или номер заказа),
		уникален в пределах заказа. Идентификатор заказа в ИС Клиента.

		:param barcode: Штрих-код упаковки, идентификатор грузоместа. Параметр используется для оперирования
		грузом на складах СДЭК, уникален в пределах заказа. Идентификатор грузоместа в ИС Клиента.
	...
	def add_item(
		...
		:param comment: Наименование товара (может также содержать описание товара: размер, цвет)
		"""
		item_element = ElementTree.SubElement(
			package_element,
			'Item',
			Amount=str(amount))
		item_element.attrib['Weight'] = str(weight)
		item_element.attrib['WareKey'] = ware_key
		item_element.attrib['Cost'] = str(cost)
		item_element.attrib['Payment'] = str(payment)
		item_element.attrib['Comment'] = comment

		return item_element

	@staticmethod
	def add_service(order_element: SubElement, code: int, count: Optional[int] = None) -> SubElement:
		"""
