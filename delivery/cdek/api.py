...
from .utils import clear_dict, get_secure, xml_to_dict, xml_to_string
...
class CDEKClient:
	...
	def __init__(
			self, account: str, secure_password: str,
	...
	def _exec_request(
			self, url: str, data: Dict, method: str = 'GET',
			stream: bool = False, **kwargs) -> requests.Response:
		if isinstance(data, dict):
			data = clear_dict(data)
		url = self._api_url + url
		if method == 'GET':
			response = requests.get(f'{url}?{urlencode(data)}', stream=stream, **kwargs)
		elif method == 'POST':
	...
	def _exec_xml_request(
			self, url: str, xml_element: Element,
	...
	def get_shipping_cost(
			...
			services: List[dict] = None) -> Dict:
		"""
		Расчет стоимости и сроков доставки. Для отправителя и получателя обязателен один из параметров:
	...
	def get_delivery_points(
		...
		"""
		Список ПВЗ. Возвращает списков пунктов самовывоза для указанного города, либо для всех если город не указан
	...
	def get_regions(
			self, region_code_ext: Optional[int] = None,
			region_code: Optional[int] = None,
			page: int = 0, size: int = 1000) -> List[Dict]:
		"""
		Список регионов. Метод используется для получения детальной информации о регионах.
	...
	def get_cities(
			...
			page: int = 0, size: int = 1000) -> List[Dict]:
		"""
		Список городов. Метод используется для получения детальной информации о городах.
	...
	def create_orders(self, delivery_request: DeliveryRequest):
		...
		return [xml_to_dict(order) for order in xml.findall('*[@DispatchNumber]')]

	def delete_orders(self, act_number: str, dispatch_numbers: List[str]) -> List[Dict]:
		"""
		Удаление заказа.

		:param str act_number: Номера акта приема-передачи. Идентификатор заказа в ИС клиента СДЭК.
		...
		for dispatch_number in dispatch_numbers:
			ElementTree.SubElement(delete_request_element, 'Order', DispatchNumber=dispatch_number)
		xml = self._exec_xml_request(url=self.DELETE_ORDER_URL, xml_element=delete_request_element)
		return [xml_to_dict(order) for order in xml.findall('*[@DispatchNumber]')]

	def call_courier(self, call_courier: CallCourier) -> Dict:
		"""
		Вызов курьера. Вызов курьера для забора посылки у ИМ
		...
		:rtype: dict
		"""
		xml = self._exec_xml_request(url=self.CALL_COURIER_URL, xml_element=call_courier.to_xml())
		return xml_to_dict(xml.find('Call'))

	def create_prealerts(self, pre_alert: PreAlert):
		"""Создание преалерта. Метод для создания сводного реестра (преалерта), содержащего все накладные,
		товары по которым передаются в СДЭК на доставку.

		:return: Результат создания преалерта
		"""
		xml = self._exec_xml_request(self.PREALERT_URL, xml_element=pre_alert.to_xml())
	...
	def get_orders_statuses(self, orders_dispatch_numbers: List[int], show_history: bool = True) -> List[Dict]:
		"""
		Статусы заказов
		...
		:returns list
		"""
		status_report_element = ElementTree.Element('StatusReport', ShowHistory=str(show_history))
		for dispatch_number in orders_dispatch_numbers:
			ElementTree.SubElement(status_report_element, 'Order', DispatchNumber=str(dispatch_number))
		xml = self._exec_xml_request(url=self.ORDER_STATUS_URL, xml_element=status_report_element)
		return [xml_to_dict(order) for order in xml.findall('Order')]

	def get_orders_print(self, orders_dispatch_numbers: List[int], copy_count: int = 1) -> Optional[requests.Response]:
		"""
	...
	def get_barcode_print(self, orders_dispatch_numbers: List[int], copy_count: int = 1) -> Optional[requests.Response]:
		"""
		Печать этикетки. Метод используется для формирования печатной формы этикетки для упаковки в формате pdf.
